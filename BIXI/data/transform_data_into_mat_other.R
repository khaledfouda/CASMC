...
#print(names(high_corr_indices))
#saveRDS(names(high_corr_indices), "./BIXI/data/splits/effective_stations.rds")

 high_corr_indices <- c( #high_corr_indices,
                         sample(1:ncol(Y),
                                floor(ncol(Y)*.5), replace=FALSE))
m <- nrow(Y)
missing_rate <- .98
test_mask <- matrix(1, nrow(Y), ncol(Y))
for (j in unique(high_corr_indices)) {
...
#tmp <- masks$miss; masks$miss <- masks$test; masks$test <- tmp
