---
title: "August 9th -BIXI Analysis notes - Location Covariates - see Obsidian"
author: "Khaled Fouda"
date: ""
output:
  html_document:
...
# dat2 <- load_model_bixi_dat3(time_cov = TRUE, seed = 2023,validp = .2) 
# dat$X = dat2$X
# dat$Y <- dat$Y %>% t()
# dat$O <- dat$O %>% t()
# dat$W <- dat$W %>% t()

# dat$fit_data$Y <- dat$fit_data$Y %>% t()
# dat$fit_data$train <- dat$fit_data$train %>% t()
# dat$fit_data$W_valid <- dat$fit_data$W_valid %>% t()
# dat$fit_data$valid <- dat$fit_data$Y[dat$fit_data$W_valid==0]
#--------------------------------------------
...
hpar_l$laplacian$lambda.a = .7778
...
out0$results$rmse_using_m <-
  out4$rmse_using_m <- out5$rmse_using_m <- NA
...
bktr.fit <- readRDS("./BIXI/data/fits/split_L_fit.rds")
test.df <-  readRDS(paste0("./BIXI/data/splits/split_L_test.rds"))
...
all.out %<>%
 rbind(
  c("BKTR",6426,NA,NA,round(unlist(prepare_output_bixi(Sys.time(), dat$X, test.estimates$y_est,
...
bktr.fit$beta_estimates %>%
 ...
 mutate(prop_non_zero = 1, Model = "BKTR")
  ...
  pack_rows("CASMC-Ridge", 1, 8, hline_after = TRUE) %>%
  pack_rows("CASMC-Nuclear", 9, 16, hline_after = TRUE) %>%
  pack_rows("CASMC-Lasso", 17, 24, hline_after = TRUE) %>% 
  pack_rows("BKTR", 25, 32, hline_after = TRUE)
 ...
 plot_model <- function(model_name) {
  ...
  ggplot(aes(x = station_id, y = value, color = covariate, group = covariate)) +
    ...
    facet_wrap(~ covariate, scales = "free_y") +
     labs(x = "Time (Day)", y = "Normalized Departures") +
    theme_minimal() +
    ggtitle(model_name) +
...
```{r}
gridExtra::grid.arrange(p3, p4, ncol = 1, nrow = 2)
