...
print_table <- function(results, missing=FALSE, prob=NA, caption=""){
   ...
   results %<>%
      ...
      mutate(missing_prob = prob, new.Method = "New", Ysf.Method="Old", simpute.Method="Simpute", simputeCov.Method="Simpute_Cov")
   ...
   sim_results$simpute.lambda.2 = -1
   sim_results$simpute.alpha = -1
   simcov_results$simputeCov.lambda.2 = -1
   simcov_results$simputeCov.alpha = -1
   ...
   sim_results$Method <- 'SImpute'
   simcov_results$Method <- 'SImpute_Cov'
   
   combined_results <- rbind(new_results, ysf_results, sim_results, simcov_results) %>% 
      arrange(Dim) %>% 
      select(-Dim) %>%
   ...
   combined_results %<>% relocate(Method, .before=true_rank) %>%
      ...
      mutate(beta = ifelse(is.na(beta), 100, beta),B = ifelse(is.na(B), 100, B))
