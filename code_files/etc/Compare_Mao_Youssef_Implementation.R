...
compare_and_save <- function(missingness,coll=TRUE, 
   ...
   if(missingness == 0){
      ...
      filename = paste0("compare_Mao_Youssef_Implementation_YsfSim_",round(missingness*100),
                        "_coll_",coll, ".csv")
   #filename = "compare_Mao_Youssef_Implementation_YsfSim_90.csv"
...
compare_and_save(0.8, TRUE, 
                 lambda.1_grid = seq(0,15,length=60),lambda.2_grid = seq(.9, 0, length=20),alpha_grid = c(1))
...
compare_and_save(0.9, FALSE,
                 lambda.1_grid = seq(0,15,length=60),lambda.2_grid = seq(.9, 0, length=20),alpha_grid = c(1))
#compare_and_save(0, lambda.1_grid = seq(0,3,length=20),
#                 lambda.2_grid = seq(.9, 0, length=20),
#                 alpha_grid = seq(0.992, 1, length=10))