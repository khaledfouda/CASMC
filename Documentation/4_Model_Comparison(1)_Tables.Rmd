...
print_table <- function(results, missingness=NULL, coll=TRUE, prob=NA, caption=""){
   ...
   for(i in seq(1,32,8))
      combined_results[i:(i+7), ] <- color_min_values(combined_results[i:(i+7), ], error_columns)
   ...
   kable("html", escape=FALSE, align = 'c', caption=caption) %>%
     ...
     pack_rows("400x400", 1, 8) %>%
     pack_rows("600x600", (8*1)+1, 8*2 ) %>% 
     pack_rows("800x800", (8*2)+1, 8*3) %>% 
     pack_rows("1000x1000", (8*3)+1, 8*4) %>%
