...
cv_ratio_SMC_x = SMCfit_cv(Y_data, X_data, mask, nfolds = 5, 
                           tau1_grid = seq(0, 2, length = 10), tau2_grid = seq(0.9, 0.1, length = 10),
                           alpha_grid = alpha_grid)  # Choose the tuning parameter by cross-validation.
...
if(cv_ratio_SMC_x$cv_min < cv_ratio_SMC_z$cv_min){
  ...
  fit_mao_z <- SMCfit(output_z*t_mask/(1+alpha), Z_data, cv_ratio_SMC_z$tau_beta_ratio, cv_ratio_SMC_z$tau_svd_ratio, cv_ratio_SMC_z$alpha_ratio)
  Y_pred_z <- t(fit_mao_z$Ahat)
