...
apply_to_movies <- function(dtrain, test) {
  ...
  beta =  fit1$beta#fit2$u %*% (fit2$d * t(fit2$v))
  ...
  sout$A_hat = revertBiScaledMatrix(as.matrix(sout$A_hat), biScale.out)
  ...
  #-----------------------------------------------------------
  # apply the k-fold with CASMAC
  
  start_time <- Sys.time()
  best_fit2 = CASMC_cv_kfold_v2(dtrain.Y, X_r, dW, trace=T,print.best = TRUE,
                                n.lambda = 20, n_folds = 5, rank.limit=70, rank.step=3)
  time_5 <- round(as.numeric(
    difftime(Sys.time(), start_time, units = "secs")), 2)  
  fit1 = best_fit2$fit
  # get estimates and validate
  beta =  fit1$beta
  ...
  data.frame(
    ...
    RMSE = c(error_1, error_2, error_3, error_4, error_5),
    Rank =  c(rank_1, rank_2, rank_3, rank_4, rank_5),
    time = c(time_1, time_2, time_3, time_4, time_5)
  )
}
