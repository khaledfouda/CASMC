...
softImputeALS_L2 <-
   function(Y.train,
            ...
            rank.limit = 30) {
      if (is.na(no_cores))
         no_cores = length(lambda1.grid) + 1
      ...
      valid_errors <- unlist(lapply(model_output, function(d)
         d$error))
      best_index = which.min(valid_errors)
      best_fit <- model_output[[best_index]]
      list(best_score = valid_errors[best_index],
           best_fit = best_fit,
           lambda1 = lambda1.grid[best_index])
   }
