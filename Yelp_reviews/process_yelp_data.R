...
file_path <-
  "./Yelp_reviews/data/yelp_academic_dataset_business.json"
...
saveRDS(business_subset,
        "./Yelp_reviews/data/subset_PA/business_PA.RDS")

file_path <- "./Yelp_reviews/data/yelp_academic_dataset_review.json"
reviews <-
  lapply(readLines(file_path, warn = FALSE, n = 10), fromJSON)
...
# rename ids
user_id_map <-
  setNames(seq_along(users_philly$user_id), users_philly$user_id)
business_id_map <-
  setNames(seq_along(business_philly$business_id),
           business_philly$business_id)
map_ids <- function(ids, map) {
  unname(map[as.character(ids)])
}
reviews_philly$user_id <-
  map_ids(reviews_philly$user_id, user_id_map)
reviews_philly$business_id <-
  map_ids(reviews_philly$business_id, business_id_map)
business_philly$business_id <-
  map_ids(business_philly$business_id, business_id_map)
...
saveRDS(
  reviews_matrix,
  paste0(
    "Yelp_reviews/data/subset_PA/sample/reviews_",
    round(log10(num_users)),
    "x",
    round(log10(num_business)),
    ".RDS"
  )
)
saveRDS(
  users_philly,
  paste0(
    "Yelp_reviews/data/subset_PA/sample/users_",
    round(log10(num_users)),
    "x",
    round(log10(num_business)),
    ".RDS"
...
saveRDS(
  business_philly,
  paste0(
    "Yelp_reviews/data/subset_PA/sample/business_",
    round(log10(num_users)),
    "x",
    round(log10(num_business)),
    ".RDS"
