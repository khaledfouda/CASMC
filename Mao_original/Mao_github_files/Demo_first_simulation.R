...
rm(list = ls())
source("./Mao_original/Mao_github_files/SMC_functions.R")  # Load the auxiliary functions, and see Readme.txt for details. 
...
one.iter(seed_num = seed_num, Y = Y, X.cov = X.cov, prob.mis = prob.mis, 
    pps.prob = pps.prob, sample.size = sample.size, design = design, cv_ratio_SMC =  cv_ratio_SMC) -> out

dim(out$est.proposed)
RMSE_error(out$fit$Ahat, Y)

dim(out$fit$)
RMSE_error(out$est.proposed, target)
#--------------------------------------------------------------------------------------------------------
# Added by khaled
# Apply othwer models to the same data
